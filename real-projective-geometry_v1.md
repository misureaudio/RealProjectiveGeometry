...
forming an orbit under the Klein four-group of the symmetric group $S_4$.
...
### 2.3. Projectivities and the Fundamental Theorem
...
The proof proceeds by showing that any collineation preserves cross-ratios (on lines), and then using the Fundamental Theorem for $\mathbb{R}\mathrm{P}^1$ to show it is induced by a linear map. The key step uses the non-degeneracy of the configuration: given $n+2$ points in general position (a *projective basis*), the collineation is determined by its values on these $n+2$ points, and the linear map sending one projective basis to another induces the same transformation.
...
Desargues' theorem is a theorem *about* $\mathbb{R}\mathrm{P}^2$ but it is *not* a theorem of the abstract incidence axioms (I1)–(I3). There exist projective planes satisfying (I1)–(I3) in which Desargues' theorem fails (the non-Desarguesian planes). A projective plane is Desarguesian if and only if it is isomorphic to $\mathbb{F}\mathrm{P}^2$ for some field $\mathbb{F}$, and for $\mathbb{F} = \mathbb{R}$ we obtain $\mathbb{R}\mathrm{P}^2$. The proof for $\mathbb{R}\mathrm{P}^2$ uses the underlying vector space: the concurrency condition at $O$ translates to a linear-algebraic relation among the homogeneous coordinate vectors, and the collinearity of the intersections follows by computing determinants.
...
Pappus' theorem is stronger than Desargues' theorem: a projective plane satisfying Pappus' theorem is isomorphic to $\mathbb{F}\mathrm{P}^2$ for a *commutative* field $\mathbb{F}$. Since Desargues' theorem holds in $\mathbb{F}\mathrm{P}^2$ for any field (commutative or not), Pappus implies Desargues but not conversely.
...
### 2.6. Projective Coordinates and Synthetic Construction
...
where $A, B$ are the intersection points of the line $PQ$ with $\Omega$, and $(\cdot,\cdot;\cdot,\cdot)$ is the cross-ratio. This is the *Cayley–Klein distance*. Depending on whether $\Omega$ is real, imaginary, or degenerate, one recovers hyperbolic, Euclidean, or elliptic geometry. In the Euclidean case, $\Omega$ is the imaginary conic $x^2 + y^2 = 0$ at infinity, and the distance formula reduces to the familiar expression via the logarithm of the cross-ratio.
...
Geodesics in $\mathbb{R}\mathrm{P}^n$ are the projections of great circles on $S^n$. Each geodesic is a closed curve of length $\pi$, and two distinct geodesics intersect in exactly one point — the projective analogue of the Euclidean axiom that two distinct lines intersect in exactly one point.

### 5.2. Projective Connections and Geodesic Equivalence

A *projective connection* on a manifold is a connection whose geodesics are preserved — not the connection itself but its unparameterized geodesics. Two metrics are *projectively equivalent* if they share the same unparameterized geodesics.
...
The tangent bundle $T\mathbb{R}\mathrm{P}^n$ fits into the short exact sequence
...
#### 7.4.1. Accessibility and Controllability
...
#### 7.5.1. Convex Algebraic Geometry and Polynomial Optimization
...
#### 7.5.2. Symplectic and Hamiltonian Geometry
...
*This essay presents the author's synthesis of established results and recent developments. Claims about specific recent preprints (items 7, 13) should be verified against the published literature as it develops.*
